...
export default function ContractUI({ walletAddress, onConnect, isConnecting }: ContractUIProps) {
  ...
  const loadSelectedWallet = useCallback(async (walletId: string) => {
    try {
      ...
      if (result) {
        const r = result as { signers: { toString(): string }[]; threshold: number };
        setSelectedWalletInfo({ signers: r.signers.map(s => s.toString()), threshold: r.threshold });
      }
    } catch { /* ignore */ }
  }, [walletAddress]);
  ...
  const handleCreateWallet = useCallback(async () => {
    ...
    try {
      await createWallet(walletAddress, newWalletId.trim(), signerList, threshold);
  ...
  }, [walletAddress, newWalletId, newSigners, newThreshold, loadWallets]);
  ...
  const handlePropose = useCallback(async () => {
    ...
    setTxStatus("Awaiting signature...");
    try {
      await proposeTransfer(walletAddress, selectedWallet, proposeTarget.trim(), BigInt(parseFloat(proposeAmount) * 1e7));
  ...
  const handleSign = useCallback(async (txIndex: number) => {
    ...
    setIsSigning(null);
  }, [walletAddress, selectedWallet, loadTransactions]);
      ...
      {txStatus && (
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-[#34d399]/15 bg-[#34d399]/[0.05] px-4 py-3 backdrop-blur-sm shadow-[0_0_30px_rgba(52,211,153,0.05)] animate-slide-down">
          <span className="text-[#34d399]">
            {txStatus.includes("on-chain") || txStatus.includes("proposed") ? <CheckIcon /> : <SpinnerIcon />}
          </span>
            ...
            {activeTab === "wallets" && (
              <div className="space-y-6">
                ...
                {selectedWallet && selectedWalletInfo && (
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-3 animate-fade-in-up">
                    <div className="flex items-center justify-between">
                ...
                {!selectedWallet ? (
                  ...
                  <>
                    ...
                    <div className="space-y-4">
                      ...
                      <Input label="Amount (XLM in Stroops, 1 XLM = 10,000,000)" value={proposeAmount} onChange={(e) => setProposeAmount(e.target.value)} placeholder="e.g. 10000000 = 1 XLM" type="number" />
                      ...
                      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-2">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex items-center justify-between font-mono text-xs">
                            ...
                            <span className="text-white/70">{(parseFloat(proposeAmount) / 1e7).toFixed(7) || "0"} XLM</span>
                          </div>
                          <div className="flex items-center justify-between font-mono text-xs">
                ...
                {!selectedWallet ? (
                  ...
                  <>
                    ...
                    {isLoadingTxs ? (
                      ...
                      <div className="space-y-3">
                        {txs.map(({ index, info }) => {
                          const hasVoted = walletAddress && (info.signed_count > 0 || info.executed);
                          return (
